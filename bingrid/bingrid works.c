   ...
   do {
      ...
      for (int row = 0; row < brd -> sz; row++){
         for (int col = 0; col < brd -> sz; col++){
            pairs_algo(brd, ONE, ZERO, row, col);
            pairs_algo(brd, ZERO, ONE, row, col);
            // printboard(brd);
            // printf("after pairs\n");
            oxo_algo(brd, ZERO, ONE, row, col);
            oxo_algo(brd, ONE, ZERO, row, col);
            // printboard(brd);
            // printf("after oxo\n");
         }
      }
...
void pairs_algo(board* brd, const char condition, const char fill, const int i, const int j){
   if (brd -> b2d[i][j] == condition){
      if (j > 0){ // horizontal check
         if (brd -> b2d[i][j] == brd -> b2d[i][j+1]){ //if prev is in bounds then apply
            brd -> b2d[i][j-1] = fill;
         }
      }
      if ((j + 2) < brd -> sz){   // if next + 1 in bounds then apply
         if (brd -> b2d[i][j] == brd -> b2d[i][j+1]){ 
            brd -> b2d[i][j+2] = fill;
         }
      }
      if (i > 0){ // horizontal check
         if (brd -> b2d[i][j] == brd -> b2d[i+1][j]){ //if prev is in bounds then apply
            brd -> b2d[i-1][j] = fill;
         }
      }
      if ((i + 2) < brd -> sz){   // if next + 1 in bounds then apply
         if (brd -> b2d[i][j] == brd -> b2d[i+1][j]){ 
            brd -> b2d[i+2][j] = fill;
         }
      }
      // if (brd -> b2d[i][j] == brd -> b2d[i+1][j]){ // vertical check
